...
void loop(void) {
  ...
  if (val == HIGH && pirState == LOW ) {
    ...
    mqttclient.publish("key/xxxxxxx/device/switch/tag/movement","{\"value\": \"1\"}", true);
    pirState = HIGH;
    delay(30*1000); //10 seconds on hold   
  } else if (pirState == HIGH) {
    Serial.println("Motion ended!");
    mqttclient.publish("key/xxxxxxx/device/switch/tag/movement","{\"value\": \"0\"}", true);
    pirState = LOW;
  }
